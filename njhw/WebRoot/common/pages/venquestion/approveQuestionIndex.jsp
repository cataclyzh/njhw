...
<form id="queryForm" action="queryApprovingQuestions.act" method="post" autocomplete="off">
   <h:panel id="query_panel" width="100%" title="查询条件">		
	...
	<h:page id="list_panel" width="100%" title="结果列表"
		customButton="[{\"name\":\"审批\",\"function\":\"approveWith();\",\"class\":\"icon-xbjbutton\"}]">
		<div style="OVERFLOW-y:auto;overflow-x:hidden;">
			<d:table name="page.result" id="row"  export="false" class="table" requestURI="">
				...
				<d:col class="display_centeralign" title="选择  " >
					<input type="checkbox" name="_chk" id="_chk" class="checkItem" value="${row.QUEST_ID}"/>
				</d:col>	
			    <d:col property="QUEST_CODE"   class="display_centeralign"   title="问题号"/>
			    <d:col property="QUEST_TYPE" class="display_leftalign" title="问题帖分类"/>
...
//查看明细操作
	function detail(id){
		var url = "${ctx}/common/question/approveQuestionDetail.act?questId="+id;
		openEasyWin("approve_question_detail","问题贴信息",url,"850","480",true);
...
function approveWith(){
   if($("input[@type=checkbox]:checked").size()==0){
		easyAlert('提示信息','请勾选要审核的记录！','info');
	}else if($("input[@type=checkbox]:checked").size()>1){
		easyAlert('提示信息','只能勾选一条要审核的记录！','info');
	}else{
		...
		var url = "${ctx}/common/question/approveInit.act?questId="+questId;
		openEasyWin("approve_question_init","问题贴信息",url,"850","480",true);
	}
}
</script>
