...
<input type="hidden" name="flag" id="flag" value="${flag }"/>
   <h:panel id="query_panel" width="100%" title="查询条件">		
	 <table align="center" id="hide_table"  border="0" width="100%" class="form_table">
		   <tr>
			...
			<tr>
				...
				<td width="30%">
					<span class="form_label">
					  	  <s:select name="questStatus"  id="questStatus"  headerKey="" headerValue="全部" cssClass="input180box"  emptyOption="false" 
						      list="#{'1':'待回复','2':'已回复','3':'已结贴'}"/>
					</span>
				</td>
	...
	<h:page id="list_panel" width="100%" title="查询结果"
		customButton="[{\"name\":\"结贴\",\"function\":\"dealWith();\",\"class\":\"icon-xbjbutton\"}]">		
...
//查看明细操作
	function detail(id){
		var url = "${ctx}/common/dealQuestion/detail.act?questId="+id;
		openEasyWin("deal_question_detail","详细信息",url,"850","480",true);
	}

	//结贴操作
	function dealWith(){
		if($("input[@type=checkbox]:checked").size()==0){
			easyAlert('提示信息','请勾选要结贴的记录！','info');
		}else{
			easyConfirm('提示', '确定结贴吗？', function(r){
				if (r){
					disableButton();
					 var chkObj = $("input[@type=checkbox]:checked");
					 var chkHid = $("._chkHidCss");
					 var strChk = "";
				  	 ...
				  	 $.each(chkHid, function(i,itema){
				  		 strChk += ($(itema).val()+",");  
				  	 });
				  	window.location = "${ctx}/common/dealQuestion/deal.act?questionIds="+strChk;
				}
			});		
		}
	}
