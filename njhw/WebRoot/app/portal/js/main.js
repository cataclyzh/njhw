...
window.onload = function ()
{
	...
	document.getElementById("img_select").src = _ctx+"/app/portal/images/shortcut/select.png";
	if(screenWidth > 1024)
	{
	...
	//用户信息
//	var url = _ctx+'/app/auth/index/userInfo.act' + '?t=' + Math.random();
...
//		document.getElementById("user_name").getElementsByTagName("span")[0].innerHTML = "当前用户：" + xmlDoc.getElementsByTagName("display_name")[0].text;
...
//		 easyAlert("提示信息", "加载用户信息出错误！");
//	};
//	ajaxQueryXML(url,null,sucFun,errFun);
	
	//一级菜单
	//url = 'Server/GetMenu.aspx?'+user_date + '&command=main_menu&nodeid=-1' + '&t=' + Math.random();
	//var xml = GetInfoFromServer(url);
	var url = _ctx+"/app/auth/index/mainMenu.act"
	var data = "pid=-1";
	var xmlDoc1;
	var sucFun = function(data){
		var xmlDoc1 = LoadXml(data);
		if(xmlDoc1 == null)
		{
			alert(xml);
			return false;
		...
		for(i = 0;i < xml_len;i++)
		{
			...
			header_ul.innerHTML += "<li class='li_dom'>"
				...
				+ "onmousedown='header1_cik(this,true)'"
				+ "IsLink='" + unescape(xmlDoc1.getElementsByTagName("IsLink")[i].text) + "'"
				+ "mainurl='" + unescape(xmlDoc1.getElementsByTagName("url")[i].text) + "'"
	...
	var errFun = function(data){
		 easyAlert("提示信息", "加载一级菜单出错！","info");
	};
	ajaxQueryXML(url,data,sucFun,errFun);
	var linkID = Request.QueryString['id'] ? Request.QueryString['id'] : "";
	var linkTitle = Request.QueryString['title'] ? decodeURI(Request.QueryString['title']) : "";
	var linkUrl = Request.QueryString['url'] ? Request.QueryString['url'] : "";
	if(linkID !="" && linkTitle!="" && linkUrl!="")
	{
		var evt = {id:linkID,title:linkTitle,mainurl:linkUrl};
		open_main2(evt, evt.id)
	}
	//herb shortcut
	//url = 'Server/GetMenu.aspx?command=shortcut_menu&'+user_date + '&t=' + Math.random();
	//var xml = GetInfoFromServer(url);
	var url = _ctx+'/app/auth/index/shortcut.act' + '?t=' + Math.random();
	...
	if(screenWidth <= "1024")
	{
		...
		newUrl = _ctx+'/app/common/homeNew.jsp' + '?t=' + Math.random();
	}
	else
	{
		...
		news_main1_main.src = _ctx+'/app/common/homeNew.jsp' + '?t=' + Math.random();
	...
	var dom = document.getElementById("header_main").getElementsByTagName("a");
	if(dom.length > 0)
	{
		firstId = dom[0].id;
		open_main2(dom[0], dom[0].id);
	}
}
...
{
	document.getElementById("footer_btn1").style.background = "url(images/main/begin.png) 0 0 no-repeat";
}
// 开始按钮颜色切换
	...
	function callback(xml)
	{
		...
		for(var i = 0,max = xmlDoc1.getElementsByTagName("title_name");i < max.length; i++)
		{
			...
			document.getElementById(mes).innerHTML += "<li uid='" 
				...
				+ " onmouseout='newslist_change(this,1)'><a href='javascript:void(0);' onmousedown='return false'>" + news_val + "</a></li>"
		}
		if(document.getElementById("btn_news_main2_" + ent.id).getElementsByTagName("li").length == 0)
		{
			document.getElementById("btn_news_main2").style.display = "none";
...
function open_main3(linkID, linkTitle, linkUrl)
{
	if(linkID !="" && linkTitle!="" && linkUrl != "")
	{
		//var evt = {"id":linkID,"title":linkTitle,"mainurl":linkUrl};
		...
		{
			open_main2(document.getElementById(linkID), document.getElementById(linkID).id)
		}
		else
		{
			...
			div.id = linkID;
			div.title = linkTitle;
			attr(div, "mainurl", linkUrl);
			div.style.display = "none";
			document.body.appendChild(div);
			open_main2(div, div.id)
		}
	}
}
// 弹出新闻关闭按钮效果
...
var header_over_ent;
function header1_cik(ent,isclick)
{
	if (isclick == true){
	...
	if(isFirefox = navigator.userAgent.indexOf("Firefox") > 0)
		var left_len = ent_parent.offsetLeft + ent_half + 365;
	else
	{
		if(ie6)
			var left_len = ent_parent.offsetLeft  + ent_half;
		else
			var left_len = document.getElementById("header").offsetLeft + ent_parent.offsetLeft + ent_half + 375;
	}
	...
	var url = _ctx+"/app/auth/index/mainMenu.act?pid="+ ent.id + '&t=' + Math.random();// 获取第二级主菜单
	var data = "pid=-1";
	var sucFun = function(xml)
	{
		var new_id = "header2_" + ent.id;
		var xmlDoc1 = LoadXml(xml);
		if(xmlDoc1 == null)
...
function header2_cik(ent)
{
	...
	//var url = 'Server/GetMenu.aspx?'+user_date + '&command=main_menu&nodeid=' + ent.id + '&t=' + Math.random();// 获取第二级主菜单
	//herb 三级导航异步加载
	var url = _ctx+"/app/auth/index/mainMenu.act?pid="+ ent.id + '&t=' + Math.random();// 获取第三级主菜单
	var sucFun = function(xml)
	{
		var xmlDoc1 = LoadXml(xml);
		if(xmlDoc1 == null)
...
function img_over(cur, which)
{
	if(getDefaultStyle(cur.parentNode,"backgroundImage").indexOf("null.png") != -1)
		return;
	if(which == 0)
		cur.src = _ctx+"/app/portal/images/shortcut/select.png";
	if(which == 1)
		cur.src = _ctx+"/app/portal/images/main/lemo.png";
}
...
function tab_change(evt)
{
	...
	if(sub != "img")
	{
		...
		{
			if(document.getElementById(evt.id.replace('label_','')))
			{
				//op.src = 'Support/TopMenu.aspx?leftid=' + (evt.id).replace('label_','') + '&t=' + Math.random();
				op.src = attr(document.getElementById(evt.id.replace('label_','')), 'mainurl');
				document.getElementById("table_body1").appendChild(op);
			}
			else
			{
				// 操作iframe
				var winframe = document.getElementById("message_iframe").contentWindow;
				if(winframe.document.getElementById(evt.id.replace('label_','')))
				{
					//op.src = 'Support/TopMenu.aspx?leftid=' + (evt.id).replace('label_','') + '&t=' + Math.random();
					op.src = attr(winframe.document.getElementById(evt.id.replace('label_','')), 'mainurl');
					document.getElementById("table_body1").appendChild(op);
				}
				else
				{
					op.src = Request.QueryString['url'];
					document.getElementById("table_body1").appendChild(op);
				}
			}
		}
...
{	
	var nowId = evt.parentNode.parentNode.id.replace("label_","")
	if(nowId == firstId)
		return false;
