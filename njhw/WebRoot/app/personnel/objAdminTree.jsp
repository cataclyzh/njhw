...
//	tree.enableContextMenu(menu);
			...
			if("${resType}"=="D"){
				if(!this.hasChildren(id)){
					...
					if(url!=null&&url!=""){
						//parent.frames.ifrmOrgList.location.href = url;
						//alert(url);
						//alert(parent.$("#ifrmObjList").attr('src'));
						parent.$("#ifrmObjList").attr('src',url);
					}
			}
		}
	);
	...
	tree.setXMLAutoLoading("${ctx}/app/per/objAdminTreeDataDev.act?resType=${resType}&type=open");
	tree.loadXML("${ctx}/app/per/objAdminTreeDataDev.act?resType=${resType}&type=init&id=<c:out value='${_orgid}'/>");//
