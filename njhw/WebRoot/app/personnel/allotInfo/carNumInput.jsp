	...
	<script type="text/javascript">	
		...
		function saveData(){
			...
			if (len == 0) alert("请输入信息后保存");
				...
				$.ajax({
					type: "POST",
					url: "${ctx}/app/personnel/validAllCarNum.act",
					...
					success: function(json){
						 if (json.status == 1) { 	//存在已分配的车配就显示
							 easyConfirm("提示信息", 
								"车牌号："+json.carNums+"已分配给其他委办局<br/>"+
								"点击确认，将会跳过已分配的车牌号保存<br/>"+
								"点击取消，退出保存",
								function(r){
									if (r) {
		...
		function loadCarNumInfo(idx) {
			...
			if (resName != "") {
				$.ajax({
					type: "POST",
					url: "${ctx}/app/personnel/loadCarNumInfo.act",
					data: "resName="+resName,
					dataType: 'json',
					async : false,
					success: function(json){
						 if (json.status == 1) { 	//已分配就显示提示信息
			    			$("#showinfo"+idx).text("此车牌号已分配给："+json.orgName);
			    			$("#showinfo"+idx).show();
			    			$("#savebut").hide();
			    			return false;
			    		} else {
							if (isRepeat(idx)) {	// 判断当前车牌是否重复
								$("#showinfo"+idx).text("车牌号重复");
				    			$("#showinfo"+idx).show();
				    			$("#savebut").hide();
				    			return false;
							}
...
<form  id="inputForm" action="save.act"  method="post"  autocomplete="off" >
	...
	<tr>
        <td class="form_label"><font style="color:red">*</font>车牌号：</td>
        <td id="addtd"> 
        	　一：<input id="resName1" theme="simple" cssClass="textbox" onblur="loadCarNumInfo('1')"/> 
        	<label id="showinfo1" style="display: none"></label><br/>
        	　二：<input id="resName2" theme="simple" cssClass="textbox" onblur="loadCarNumInfo('2')"/> 
        	<label id="showinfo2" style="display: none"></label><br/>
        	　三：<input id="resName3" theme="simple" cssClass="textbox" onblur="loadCarNumInfo('3')"/> 
        	<label id="showinfo3" style="display: none"></label><br/>
        	　四：<input id="resName4" theme="simple" cssClass="textbox" onblur="loadCarNumInfo('4')"/> 
        	<label id="showinfo4" style="display: none"></label><br/>
        	　五：<input id="resName5" theme="simple" cssClass="textbox" onblur="loadCarNumInfo('5')"/> 
        	<label id="showinfo5" style="display: none"></label><br/>
        	　六：<input id="resName6" theme="simple" cssClass="textbox" onblur="loadCarNumInfo('6')"/> 
        	<label id="showinfo6" style="display: none"></label><br/>
        	　七：<input id="resName7" theme="simple" cssClass="textbox" onblur="loadCarNumInfo('7')"/> 
        	<label id="showinfo7" style="display: none"></label><br/>
        	　八：<input id="resName8" theme="simple" cssClass="textbox" onblur="loadCarNumInfo('8')"/> 
        	<label id="showinfo8" style="display: none"></label><br/>
        	　九：<input id="resName9" theme="simple" cssClass="textbox" onblur="loadCarNumInfo('9')"/> 
        	<label id="showinfo9" style="display: none"></label><br/>
        	　十：<input id="resName10" theme="simple" cssClass="textbox" onblur="loadCarNumInfo('10')"/>
        	<label id="showinfo10" style="display: none"></label>
        </td> 
        <td id="updatetd">
        	<input id="resName1" theme="simple" cssClass="textbox" onblur="loadCarNumInfo('1')" value="${emOrgRes.resName }"/> 
