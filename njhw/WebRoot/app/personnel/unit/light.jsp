...
<html>
						...
						$.ajax({
							...
							success: function(msg){
								  if(msg == "success") {
										...
										//resultInfo("成功开锁!",roomId)
									  	setTimeout("close_door('"+roomId+"')", 3000);	// 开门3秒之后关门
								  } else 
								  {
								  	//resultInfo("开锁失败!",roomId)
    		...
    		<c:if test="${data.RES_ID != '0'}">
		    	<div class="main_body" >
		            <div class="main_header">
		               	<div title="无电话接入"  roomId = "${data.RES_ID}"  <c:if test="${data.INNER_STATUS=='1'}">class="main_tel3"</c:if> <c:if test="${data.INNER_STATUS=='2' || data.INNER_STATUS=='3'}">class="main_tel"</c:if>  style="visibility:hidden" ></div>
		                <div class="main_num">
		                    <span title = "${data.KEYWORD}">${fn:substring(data.KEYWORD,0,6)}</span>
		                </div>
		                <div title="" roomId = "${data.RES_ID}" id="roomId${data.RES_ID}" onclick="javascript:light('${data.RES_ID}');"><img width="24" hight="24" id="img${data.RES_ID}" src="${ctx}/app/personnel/unit/images/light_on.png"></img></div>
		                <input type = "hidden" id = "light${data.RES_ID}" value="on">
		            </div>
		          
		            <div class="room_name">
		                <!--<a href="javascript:void(0);" hidefocus title="${data.TITLE}">--><div onclick="" style="font-size: 16px;font-family:"Microsoft YaHei",微软雅黑,"Microsoft JhengHei",华文细黑,STHeiti,MingLiu;color: #154ac0;font-weight: bold;">${fn:substring(data.TITLE,0,6)}</div><!--</a>-->
		            </div>
		            <!--
		            <div>
		            	<div style=" float:left; display:inline" title="灯">灯
		            	<image src="${ctx}/app/personnel/unit/images/door.png" />
		            	</div>
		            	<div style="float:right; display:inline" title="空调">空调
		            	<image src="${ctx}/app/personnel/unit/images/door.png" />
		            	</div>
		            </div>-->
		            <div class="people">
		               	<div <c:if test="${data.USER_NAMES == ''}">class="nopeople_img"</c:if> <c:if test="${data.USER_NAMES != ''}">class="people_img"</c:if> roomId = "${data.RES_ID}" title=""  ></div>
		                ...
		                <div id="close_door_${data.RES_ID}" title = "" class="close_door"  onclick="airConditioning('${data.RES_ID}');">空调</div>
		                <div id="open_door_${data.RES_ID}" title = "" class="open_door" style="display: none;"></div>
		            </div>
		        </div>
	        </c:if>
	        <c:if test="${data.RES_ID == '0'}">
	...
	<script type="text/javascript">
			...
			<c:forEach items="${page.result}" var = "data">
    			<c:if test="${data.RES_ID != '0'}">
					lightListStatus('${data.RES_ID}');
			    </c:if>
    		</c:forEach>
			
				
			function lightListStatus(roomId)
        	{
        		//alert(roomId);
            	$.ajax({
						type: "POST",
						url: "${ctx}/app/integrateservice/jsonLightStatus.act?roomId="+roomId,
						data: {"roomId":roomId},
						dataType: 'json',
						async : false,
						success: function(msg){
							if (msg.status=="on")
							{
								document.getElementById("img"+roomId).src="${ctx}/app/personnel/unit/images/light_on.png";
							}
							else
							{
								document.getElementById("img"+roomId).src="${ctx}/app/personnel/unit/images/light_off.png";
							}
			...
			{
				//alert("你点击了灯");
				var lightStatus=document.getElementById("light"+roomId).value;
				//alert(lightStatus);
				if (lightStatus=="on")
				{
					lightStatus="off";
				...
				$.ajax({
					type: "POST",
					url: "${ctx}/app/integrateservice/jsonLight.act?roomId="+roomId,
					data: {"deviceType": "light","roomId":roomId,"optType":lightStatus},
					dataType: 'json',
					async : false,
					success: function(msg){
						...
						{
							//alert("111");
							document.getElementById("light"+roomId).value = lightStatus;
							//alert("333");
							if (lightStatus =="on")
							{
								//document.getElementById("img"+roomId).innerHTML= "开灯";
							...
							{
								document.getElementById("img"+roomId).src="${ctx}/app/personnel/unit/images/light_off.png";
								//document.getElementById("roomId"+roomId).innerHTML= "关灯";
								alert("关灯成功");
							}
			...
			function airConditioning(roomId)
			{
				alert("你点击了空调");
				$.ajax({
					type: "POST",
