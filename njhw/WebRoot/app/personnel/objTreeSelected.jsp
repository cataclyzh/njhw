...
<form  id="inputForm" action="orgUserSelectSave.act"  method="post"  autocomplete="off" >
...
//	tree.enableContextMenu(menu);
	...
	tree.setXMLAutoLoading("${ctx}/app/per/objTreeSelectData.act?type=open");
	tree.loadXML("${ctx}/app/per/objTreeSelectData.act");
	...
	function ajaxTest(){
  		$.post("/app/per/objUserSelectSave.act", {"txt": "123" },function(data){
   			alert(data);
  		});
 	}
	...
	function saveData(){
		...
		//alert("checkedIds:"+$('#checkedIds').val());
		var id = $('#checkedIds').val();
		parent.window.frames["ifrmRoomList"].addNode(id);
//		frm.submit();
	}
</script>
</body>
