...
<body leftmargin="0" topmargin="0" STYLE='OVERFLOW:HIDDEN;background: #fff;'>
...
//	tree.enableContextMenu(menu);
	...
	tree.setXMLAutoLoading("${ctx}/app/per/orgTreeUserCheckinData.act?type=open&telType=<c:out value='${telType}'/>");
	tree.loadXML("${ctx}/app/personnel/telAndNumber/orgTreeUserData.act?type=init&id=<c:out value='${_orgid}'/>&telType=<c:out value='${telType}'/>");//
	...
	function HandleTreeClk(sId,sPreId){
		...
		if(parentId)
		{
			sTxtOrg = tree.getItemText(parentId);
		}
		//alert("你点击了节点:"+sTxt+",上次点击节点:"+sTxtOrg);
		if (sId.indexOf("-u")!=-1){
			//刷新人员信息
			parent[0].refreshUser(sId,sTxt,sTxtOrg);
			parent.closeEasyWin("chooseUserWin");
		}
	}
    ...
    function findPlace(id){
         var itemId = id+'-u'
         var node=tree._globalIdStorageFind(itemId) ;//根据id获取节点
