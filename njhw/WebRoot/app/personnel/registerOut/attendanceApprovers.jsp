...
<form  id="inputForm" action="orgUserSelectSave.act"  method="post"  onkeydown= "if(event.keycode==13)return false;" >
 <s:hidden name="orgId"/>
...
//	tree.enableContextMenu(menu);
	...
	tree.loadXML("${ctx}/app/per/attendanceApproversData.act?roomId=${roomId}&dtype=${orgid}");
	...
	function saveDataFn(){
		...
		if(null == $("#checkedIds").val() || '' == $("#checkedIds").val()){
			easyAlert('提示信息','请选择考勤审批人员！','success');
			return false;
		}
		frm.submit();
		...
		$(window.top.ifrmOrgList.document.getElementById("attendance_approvers")).text(appsName.substring(0,appsName.length-1));  
	}
...
<c:if test="${isSuc=='true'}">
	...
	window.top.$("#approversInput").window("close");
</c:if>
