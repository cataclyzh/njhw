		...
		function saveData(){
		  ...
		  if ($("#inputForm").valid()) {
		  	if ($("#opt").val() == '1' && '' == $("#access").val() && '' == $("#guard").val()) {
 		    	...
 		    	frm.submit();
 		    	if ('' == $('#access').val()) {
 		    		if (window.top.ifrmOrgList != undefined) {
 		    			window.top.ifrmOrgList.$('#accFloor').html('正在删除申请中...');
 		    		} else {
 		    			window.top.njhwAdmin.ifrmOrgList.$('#accFloor').html('正在删除申请中...');
 		    		}
 		    	} else {
 		    		if (window.top.ifrmOrgList != undefined) {
 		    			window.top.ifrmOrgList.$('#accFloor').html($('#access').val() + "    正在申请中...");
 		    		} else {
 		    			window.top.njhwAdmin.ifrmOrgList.$('#accFloor').html($('#access').val() + "    正在申请中...");
 		    		}
 		    	}
 		    	if ('' == $('#guard').val()) {
 		    	    if (window.top.ifrmOrgList != undefined) {
 		    	    	window.top.ifrmOrgList.$('#accGate').html('正在删除申请中...');
 		    	    } else {
 		    	    	window.top.njhwAdmin.ifrmOrgList.$('#accGate').html('正在删除申请中...');
 		    	    }
 		    	} else {
 		    		if (window.top.ifrmOrgList != undefined) {
 		    			window.top.ifrmOrgList.$('#accGate').html($('#guard').val()+"    正在申请中...");
 		    		} else {
 		    			window.top.njhwAdmin.ifrmOrgList.$('#accGate').html($('#guard').val()+"    正在申请中...");
 		    		}
 		    	}
