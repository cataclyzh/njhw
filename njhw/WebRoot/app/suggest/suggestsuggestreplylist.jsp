...
<%@ include file="/common/include/taglibs.jsp"%>
	<div class="qbyj_center_on">
		...
		<c:choose>
			<c:when test="${sug.status=='1'}">
				...
				<a class="qbyj_no_a" href="javascript:void(0);" onclick="toggleReply('hf_'+${sug.sugid},this)">收起回复</a>
			</c:when>
			<c:otherwise>
	...
	<div class="qbyj_txt_ons" style="overflow: auto;">
		<textarea class="show_textarea" readonly="readonly" style="overflow: auto;height: 120px;width: 99%;">${sug.content }</textarea>
	</div>
				...
				<c:forEach items="${sug.suggestReplyVOList}" var="rep" varStatus="repStatus" >
					<c:choose>
						<c:when test="${repStatus.count%2==0}">
							<div class="qbyj_center_ons1" id="hf_${sug.sugid }_bt" >
							...
							<div class="qbyj_txt_ons1" id="hf_${sug.sugid }_nr" style="overflow: auto;">
								<textarea class="show_textarea" readonly="readonly" style="overflow: auto;height: 120px;width: 99%;">${rep.content }</textarea>
							</div>
						</c:when>
						<c:otherwise>
							<div class="qbyj_center_ons" id="hf_${sug.sugid }_bt" >
								<span class="qbyj_on_t">回复时间：${rep.createtimeString }</span>
								<span class="qbyj_on_t">回复人：${rep.displayName }( ${rep.shortName} )</span>
								<c:if test="${rep.userid eq _userid}">
									<a class="qbyj_no_as" href="javascript:void(0);" onclick="delSuggestReply(${rep.repid},${sug.sugid})">删除回复</a>
							...
							<div class="qbyj_txt_ons" id="hf_${sug.sugid }_nr" style="overflow: auto;">
								<textarea class="show_textarea" readonly="readonly" style="overflow: auto;height: 120px;width: 99%;">${rep.content }</textarea>
							</div>
						</c:otherwise>
					</c:choose>
