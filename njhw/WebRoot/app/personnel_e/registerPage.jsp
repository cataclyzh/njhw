	...
	<head>
		...
		$(document).ready(function() {
			...
			if ("${user.loginUid}" != "") {
				...
				$("#roomId").val("${exp.roomInfo}");
				$("#roomNum").val("${exp.roomInfo}");
				$("#ipTelNumval").val("${exp.telNum}");
				$("#sfax").val("${exp.uepFax}");
				// 门禁
...
<div class="inform_index">
	...
	<form id="inputForm" action="saveRegisterE.act" method="post" autocomplete="off">
		...
		<div class="bgsgl_conter" style="border-top:#7f90b3 1px solid">
			  ...
			  <div class="bgsgl_right_list_left">E4登录基本信息</div>
			</div>
...
<table width="100%" border="0" cellpadding="0" cellspacing="0">
  <tr>
    ...
    <td width="21%"><input type="text" class="infrom_input_s"  value='${exp.roomInfo}' name="roomNum"  id="roomNum"/>
	...
	<td width="21%"><input type="text" class="infrom_input_s"  name="ipTelId"  value = '${exp.telNum}' id="ipTelNumval" />
      
    <td width="9%"></td>
  </tr>
  <tr>
    <td><div class="infrom_td">传真号</div></td>
    <td><input type="text" class="infrom_input_s" name='fax' val="${exp.uepFax}" id="sfax" />
