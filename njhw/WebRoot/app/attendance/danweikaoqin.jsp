...
/* 			if (showRequest()) {
		...
		function gotoWodekaoqin(userId, userName){
			//var leftDoc = parent.document.getElementById("main_frame_left").contentWindow.document;
			//$(leftDoc.getElementById("btn_wodekaoqin")).parent().attr("class","m1_select");
			...
			//windowDialog("考勤明细",url,"1090","700",false);
		}
...
<body style="background: #fff;overflow-x:hidden;">
	<div class="fkdj_index">
		<div class="bgsgl_border_left">
			<!--此处写页面的标题 -->
			<div class="bgsgl_border_right">单位考勤</div>
		</div>
		 ...
		 <div class="main_conter"><!-- <<main_conter>> -->
		   <div class="main_main1" id="set_holiday_tab_nav">
			 ...
			 <div>	<!--tab content -->
				...
				<form id="queryForm" name="queryForm" action="danweikaoqin.act?opt=query" method="post">
				...
				<h:panel id="query_panel" width="100%" title="查询条件">
					<div id="hide_table">
						...
						<select id="selectDepartment_show" name="selectDepartment_show">
						<%
						...
						<input type="text" id="startTime_show" name="startTime_show" value="${startTime }" cssClass="fkdj_from_input" onclick="WdatePicker({dateFmt:'yyyy-MM-dd'})"/>
						<label>至</label>
						<input type="text" id="endTime_show" name="endTime_show" value="${endTime }" cssClass="fkdj_from_input" onclick="WdatePicker({dateFmt:'yyyy-MM-dd'})"/>
				...
				<s:textfield name="page.pageSize" id="pageSize" value="10"  theme="simple" cssStyle="display:none" onblur="return checkPageSize();" onkeyup="value=value.replace(/[^\d]/g,'');" />
 				...
 				<d:table name="page.result" id="row" uid="row" export="true" requestURI="danweikaoqin.act?type=export" cellspacing="0" cellpadding="0" class="table">
					...
					<d:setProperty name="export.excel.filename" value="单位考勤_${startTime}_${endTime}.xls"/>
							...
							<table id="counts_table" cellpadding="0" cellspacing="0">
								<tr>
									...
									<td>
										...
										onmousemove="this.style.color='#ffae00'" onclick="javascript:realTimePopWindow('2','0')"> [详情] </a>
									</td>
								</tr>
								<tr>
									<td>
									...
									<td>
										<a style="color: rgb(255, 174, 0); cursor: pointer;" 
										onmousemove="this.style.color='#ffae00'" onclick="javascript:realTimePopWindow('2','1')"> [详情] </a>
									</td>
								</tr>
							</table>
		...
		<div id="container" style="min-width: 1045px; height: 400px; margin: 0 auto;float:left;"></div>
	...
	function getCharts(){
		...
		$.ajax({
	            ...
	            dataType: 'json',
	            data: {"type": 1,"mastartTime":$("#mastartTime").val(),"maendTime":$("#maendTime").val(),},
	            complete: function(){
	            	$("#loadingImg").hide();
				},
	            beforeSend: function(){
