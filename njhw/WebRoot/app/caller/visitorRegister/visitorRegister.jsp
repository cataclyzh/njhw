...
<body topmargin="0" leftmargin="0" rightmargin="0">
 	...
 	<form id="queryForm" action="visitorList.act" method="post"  autocomplete="off">
	...
	<h:page id="list_panel" width="100%" title="结果列表">		
		<d:table name="page.result" id="row" uid="row" export="false" class="table"	requestURI="list.act?exFile=1">	
		    ...
		    <d:col class="display_centeralign"  media="html" title="操作">	
			 <a href="javascript:void(0);" onclick="updateRecord('${row.VS_ID}')">[发放临时卡]</a>&nbsp;
				  <a href="javascript:void(0);" onclick="updateRecord1('${row.VS_ID}')">[绑定副卡]</a>&nbsp;
				<a href="javascript:void(0);" onclick="updateRecord2('${row.VS_ID}')">[查看]</a>&nbsp;
			</d:col>	
...
<script type="text/javascript">
	...
	function updateRecord(id){
		//alert(id);
		//临时卡
		var url = "${ctx}/app/visitor/getvisifoByid.act?vsId="+id;
		openEasyWin("rfid","发放临时卡",url,"800","400",true);
	}
	function updateRecord1(id){
		//alert(id);
		//绑定副卡
		var url = "${ctx}/app/visitor/getvisifoByid1.act?vsId="+id;
		openEasyWin("winId","发放副卡",url,"800","400",true);
	}
	function updateRecord2(id){
		//alert(id);
		//查看
		var url = "${ctx}/app/visitor/getAllInfor.act?vsId="+id;
		
		openEasyWin("winId","查看发卡信息",url,"800","400",true);
	}
	function delRecord(){
