...
//tree.setXMLAutoLoading("/app_njhw/app/room/objTreeSelectData.act?type=open");
	tree.loadXML("/app_njhw/app/sendsms/getAddressTreeData.act?ids=${ids}");
	...
	function saveData(){
		...
		if (tree.getAllChecked() != ""){
			...
			for ( var i = 0; i < idsArray.length; i++) {
				if (idsArray[i].indexOf("_u")>0) {	//人员
					...
					infos += info;
					chooseText += "<div style='float: left; margin-right: 10px;'>"+idArray[1]+"<img src='${ctx}/app/integrateservice/images/close.png' title='删除联系人' onclick='del(this, &quot;addressTR&quot;, &quot;addressAlreadyChoose&quot;, &quot;"+info+"&quot;, &quot;addressReceiverInfo&quot;)'/></div>";
				}
			}
		}
		//alert("ids:"+ids);
		...
		if (chooseText != "")  parent.$('#addressTR').show();
		else parent.$('#addressTR').hide();
		
		parent.$('#addressAlreadyChoose').children().remove();
		parent.$('#addressAlreadyChoose').append(chooseText);
		
		//parent.$('#addressreceiverid').val(ids);
		//parent.$('#addressreceiver').val(names);
		//parent.$('#addressreceivermobile').val(mobiles);
		parent.$('#addressReceiverInfo').val(infos);
