...
<body>
	<div class="main_right3">
		...
		<div class="main_right3_more">
			<div class="main_right3_more_sz" id="peopleSet" style="display: none;float:left;right:935px;" onclick="showShelter('1010','410','${ctx}/app/paddressbook/init.act');"  title="设置"></div>
			<div class="refresh_main_right3_more" id="refresh_List" style="float:left;right:905px;" onclick="refreshAddList()" title="刷新"></div>
		</div>
		...
		<div class="search_span"><span class="num" onclick="searchInfo()" id="search">查&nbsp;找</span></div>
...
<script type="text/javascript">
        ...
        $.getJSON(firsturl, function(json){
	        ...
	        for(var i = 0; i < perlen; i++) {
	            ...
	            var unitInfo = id+"_"+name+"_"+mobile;
	             ...
	             var text = "<table width='800px'><tr><td width='15%'><span >"+name+"</span></td><td width='83%'>", title=name;
	            if ("${type}" == "tel") {
    ...
    function loadaddresstree(url, tv, gId) {
        ...
        $.getJSON(firsturl, function(json){
	        ...
	        for(var i = 0; i < orglen; i++) {
	            ...
	            var text = "<table width='1000px'><tr><td width='7%'><span >"+name+"</span></td><td width='93%' style='color: #7d7d7d;'>", title=name;
	            if (phone != "") {		// 多电话循环
	            	if (phone.indexOf(',') > 0) {
    ...
    function loadaddressnode(url, Pnode) {
	    ...
	    $.getJSON(firsturl, function(json){
	        ...
	        for(var i = 0; i < orglen; i++) {
	            ...
	            var text = "<table width='1000px'><tr><td width='7%'><span >"+name+"</span></td><td width='90%' style='color: #7d7d7d;'>", title=name;
	            if (phone != "") {		// 多电话循环
	            	if (phone.indexOf(',') > 0) {
	            		var phoneList = phone.split(',');
	            		for ( var j = 0; j <  phoneList.length; j++) {
    ...
    function dialing(called, pname) {
  		var url = "${ctx}/app/integrateservice/inputCallPhone.act?selfMac=${smac}&selfTel=${stel}&called="+called+"&pname="+pname;
		openEasyWinNotResizable("winId","拨号确认",url,"400","170",true);
	  	//$.ajax({
		//	type: "POST",
		//	url:  "${ctx}/app/integrateservice/ipCall.act",
		//	data: {"selfMac": "${smac}", "selfTel": "${stel}", "called": called},
		//	dataType: 'text',
		//	async : false,
        	...
        	$("#close_search").show();
	  		...
	  		$.ajax({
				...
				success: function(json){
					if(json.length > 0) {
						$("#searchList").append("<table style='margin-left:5px' cellspacing='20' id='contact_tab'></table>");
						if (showType == "pabList") {
