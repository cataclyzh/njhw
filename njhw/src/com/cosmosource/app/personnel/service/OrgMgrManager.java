...
public class OrgMgrManager extends BaseManager{
	 ...
	 * @param ids
	 */
	public void saveApprovers(String userid,String ids){
		StringBuffer useridArr=new StringBuffer();
		StringTokenizer st = new StringTokenizer(ids,",");
		String useridTemp = null;
		while(st.hasMoreTokens()){
			useridTemp = st.nextToken();
			if(useridTemp.endsWith("-u")){
				useridArr.append(useridTemp.substring(0, useridTemp.length()-2)).append(",");
			}
			
		}
		AttendanceApprovers aa = new AttendanceApprovers(Long.valueOf(userid),useridArr.substring(0,useridArr.length()-1));
		dao.saveOrUpdate(aa);
	}
	/**
	 * 保存大厦考勤员
	 * @param ids
	 */
	public void saveBuildingAtt(String ids){
		StringBuffer useridArr=new StringBuffer();
		StringTokenizer st = new StringTokenizer(ids,",");
		String useridTemp = null;
		while(st.hasMoreTokens()){
		...
		BuildingAttendancers aa = new BuildingAttendancers(2l,useridArr.substring(0,useridArr.length()-1));
		dao.saveOrUpdate(aa);
	}
	...
	public void addObjPermMapForDoor(String userId, String nodeId) {
		try{
			...
			addObjPermMapForDoor(userId, nodeId);
		}
	}	
	...
	public void saveOrg(Org entity){
		...
		if (null == entity.getOrgId())
		{
			Long tempOrderNum;
			if(entity.getPId() != null && entity.getPId() > 0){
				tempOrderNum = updateOrderNum(entity.getOrderNum(), null, entity.getPId());
				//entity.setOrderNum(getMaxOrg(entity.getPId()).getOrderNum()+1);
				entity.setOrderNum(tempOrderNum);
			}
			dao.save(entity);
		}
		else 
		{
			...
			if(entity.getPId() != null && entity.getPId() > 0){
				...
				Org tempOrg = (Org)dao.get(Org.class, entity.getOrgId());
				tempOrderNum = updateOrderNum(entity.getOrderNum(), tempOrg.getOrderNum(), entity.getPId());
				dao.getSession().evict(tempOrg);
				entity.setOrderNum(tempOrderNum);
			}
	...
	public Long updateOrderNum(Long currentOrderNum, Long oldOrderNum,Long pid) {
		Long maxOrdernum = getMaxOrg(pid).getOrderNum();
		Long ordernum = 1L;
		if(maxOrdernum!=null){
			...
			if(oldOrderNum==null){
				...
				if(currentOrderNum>maxOrdernum){
					...
					Map map = new HashMap();
					map.put("start", currentOrderNum);
					map.put("end", maxOrdernum+1);
					map.put("pid", pid);
					List<Map> list = new ArrayList<Map>();
					list.add(map);
					sqlDao.batchUpdate("PersonnelSQL.updateOrgOrder", list);
				}
			}else{
				if(currentOrderNum>oldOrderNum){
					if(currentOrderNum>maxOrdernum){
						currentOrderNum = maxOrdernum;
					}
					ordernum = currentOrderNum;
					Map map = new HashMap();
					map.put("start", oldOrderNum);
					map.put("end", currentOrderNum);
					map.put("pid", pid);
					List<Map> list = new ArrayList<Map>();
					list.add(map);
					sqlDao.batchUpdate("PersonnelSQL.updateOrgOrder2", list);
					
				}else if(currentOrderNum<oldOrderNum){
					ordernum = currentOrderNum;
					Map map = new HashMap();
					map.put("start", currentOrderNum);
					map.put("end", oldOrderNum);
					map.put("pid", pid);
					List<Map> list = new ArrayList<Map>();
					list.add(map);
					sqlDao.batchUpdate("PersonnelSQL.updateOrgOrder", list);
	...
	public Long deleteOrgs(String[] ids){
		...
		if(lVari==null){
			...
			for(int i=0;i<ids.length;i++){
				...
				Map map = new HashMap();
				map.put("start", ent.getOrderNum());
				map.put("pid", ent.getPId());
				List<Map> list = new ArrayList<Map>();
				list.add(map);
				sqlDao.batchUpdate("PersonnelSQL.updateOrgOrder2", list);
...
//			if(list.size()<=200){
				for (Org org : list) {
		            ...
		            if(StringUtil.trim(org.getLevelNum().toString()).equals("3")){
		            	el.addAttribute("child", "0");
		            	elx.addText(ctx+"/app/per/orgInput.act?orgId="+ org.getOrgId()+"&levelNum="+(new Long(org.getLevelNum().trim())));
		            }else{
		            	List<Org> reList =  dao.findByHQL("select t from Org t where t.PId in (select s.orgId from Org s where s.PId =? ) order by orderNum", new Long(orgid));
		            	if(reList.size() == 0){
		            		el.addAttribute("child", "0");
			            	elx.addText(ctx+"/app/per/orgInput.act?orgId="+ org.getOrgId()+"&levelNum="+(new Long(org.getLevelNum().trim())));
		            	}else{
			            	el.addAttribute("child", "1");
			            	elx.addText(ctx+"/app/per/orgList.act?orgId="+ org.getOrgId()+"&levelNum="+(new Long(org.getLevelNum().trim())));
		            	}
		            }
...
//			}
	...
	/**
	 * 
	* @Title: getOrgTreeDataAdmin 
	* @Description: 组织机构（一级单位）管理
	* @author 张权威
	* @date 2013年8月12日14:18:55
	* @param  orgid
	* @param  ctx
	* @param type
	* @return String 
	 */
	public String getOrgTreeDataAdmin(String orgid, String ctx, String type) {
		if ("init".equals(type)) {
			Document doc = DocumentHelper.createDocument();
			Element root = doc.addElement("tree");
			root.addAttribute("id", "0");
			...
			Element root= DocumentHelper.createElement("tree"); 
			root.addAttribute("id",orgid);
			if(orgid.indexOf("-o") != -1)
			{
				orgid	= orgid.substring(0, orgid.indexOf("-o"));
			}
			List<Org> list =   dao.findByHQL("select t from Org t where t.PId=? order by orderNum",new Long(orgid));
//			if(list.size()<=200){
				for (Org org : list) {
		            ...
		            if(StringUtil.trim(org.getLevelNum().toString()).equals("2")){
		            	el.addAttribute("child", "0");
		            	elx.addText(ctx+"/app/personnel/orguser/orgIndex.act?orgId="+ org.getOrgId());		
		            }else{
		            	List<Org> reList =  dao.findByHQL("select t from Org t where t.PId in (select s.orgId from Org s where s.PId =? ) order by orderNum", new Long(orgid));
...
//					TcIpTel  ttt = (TcIpTel)obj[2];
		            ...
		            //判断用户是否是E座用户
		            if(e4PersonnelManager.isE4Personnel(userId)){
		            	elx.addText(ctx+"/app/e4personnel/registerPage.act?orgId="+orgId+"&userId="+ userId);
		            }else{
...
//			}
	 ...
	 * @return
	 */
	public String getNjhwWyUserTree(String ctx){
		Document doc = DocumentHelper.createDocument();
		Element root = doc.addElement("tree");
		root.addAttribute("id", "0");

		getNjhwWyUserTree(null,root);
		return doc.asXML();		
	}
	...
	* @Description 弹出页面机构及用户树checkbox
	* @Author：zhujiabiao
	* @Date 2013-8-8 上午10:09:17 
	* @param
	* @return
	* @throws Exception
	* @version V1.0   
	**/
	public String getOrgAdminUserTreeData(String orgid, String ctx,String type, String nodeId) {		
		Document doc = DocumentHelper.createDocument();
		Element root = doc.addElement("tree");
		if(type.equals("init")){
			root.addAttribute("id", "0");
		...
		getOrgAdminUserTreeDoc(orgid,root,type,nodeId);
		return doc.asXML();		
	}
	 ...
	 * @return
	 */
	public void getNjhwWyUserTree(Long orgId,Element rootElement){
		if(null == orgId){
			List<Long> orgIds = new ArrayList<Long>();
			orgIds.add(300l);
			orgIds.add(213l);
			List<Org> orglist = dao.findByIds(Org.class, orgIds);
			for(Org org:orglist){
				 Element el=rootElement.addElement("item");   
		            el.addAttribute("text", org.getName());
		            el.addAttribute("id", org.getOrgId()+"-o");
		            getNjhwWyUserTree(org.getOrgId(),el);
			}
		}else{
			List<Org> list =  dao.findByHQL("select t from Org t where t.PId=? order by orderNum", orgId);
			...
			String buildingAtts = unitAdminManager.getBuildingAtt(2l);
			List<Users> userList = dao.findByHQL("select t from Users t where t.orgId=? order by orderNum", orgId);
			for (Users user : userList) {
					Element el=rootElement.addElement("item");   
		            el.addAttribute("text", user.getDisplayName());
		            el.addAttribute("id", user.getUserid()+"-u");
		            ...
		            el.addAttribute("im2", "user.gif");	
		            // 选中审批人员
		            if(null != buildingAtts && ArrayUtils.contains(buildingAtts.split(","), user.getUserid().toString())){
	...
	public void getOrgUserTreeDoc(String orgid,String roomid,Element rootElement, String dtype){	
		if (orgid==null) {
			...
			if("approvers".equals(dtype)){
				String approvers = unitAdminManager.getApprovers(roomid);
				List<Users> userList = dao.findByHQL("select t from Users t where t.orgId=? order by orderNum", Long.valueOf(orgid));
				for (Users user : userList) {
						Element el=rootElement.addElement("item");   
			            el.addAttribute("text", user.getDisplayName());
			            el.addAttribute("id", user.getUserid()+"-u");
			            ...
			            el.addAttribute("im2", "user.gif");	
			            // 选中审批人员
			            if(null != approvers && ArrayUtils.contains(approvers.split(","), user.getUserid().toString())){
							 el.addAttribute("checked", "1");
			            }
				}		   
			}
	...
	* @Description 弹出页面机构及用户树checkbox
	* @Author：zhujiabiao
	* @Date 2013-8-8 上午10:09:54 
	* @param
	* @return
	* @throws Exception
	* @version V1.0   
	**/
	public void getOrgAdminUserTreeDoc(String orgid,Element rootElement,String type, String nodeId){
		
		Map permParam = new HashMap();
		permParam.put("nodeId", nodeId);
		List<Map> permList = this.findListBySql("PersonnelSQL.getObjPermMap", permParam);
		
		List orgList = new ArrayList();
		List usrList = new ArrayList();
		
		for(Map m : permList){
			String permType = (String) m.get("TYPE");
			long permId = Long.parseLong(m.get("PERSON_ID").toString());
			
			if(permType.trim().equals("org")){
				orgList.add(permId);
			}else{
				usrList.add(permId);
			}
		...
		if("init".equals(type)){
			Map pMap = new HashMap();
			pMap.put("userid", 1);
			List<Org> list = this.findListBySql("PersonnelSQL.getOrgAdminByManager", pMap);
			...
			for (Org org : list) {
				...
				//el.addAttribute("open", "1");
				el.addAttribute("child", "1");
				if(orgList.contains(org.getOrgId())){
					el.addAttribute("nocheckbox", "1");
				}
				
				//getOrgAdminUserTreeDoc(org.getOrgId().toString(),el,type);
			}
		}else{
			List<Org> list =   dao.findByHQL("select t from Org t where t.PId=? order by t.orderNum ", new Long(orgid));
			...
			for (Users user : userList) {     
				Element el=rootElement.addElement("item");   
				el.addAttribute("text", user.getDisplayName());
				el.addAttribute("id", "user_" + user.getUserid()+"");
				el.addAttribute("im0", "user.gif");		
				el.addAttribute("im1", "user.gif");		
				el.addAttribute("im2", "user.gif");
				el.addAttribute("child", "0");
				if(usrList.contains(user.getUserid())){
					el.addAttribute("nocheckbox", "1");
				}
			}
		}
	...
	* @throws
	...
	public Boolean deleteUser(long userid,String loginid,DoorControlToAppService doorControlToAppService){
		...
		try{
			...
			accessMgrManager.accessAndGateOpt(userid, "2");

			delExp(userid);
			delCards(userid);
			// 删除人员类型
			delLeaderLeavel(userid);
			// 删除考核审批人员
			delAttendanceApprovers(userid);
			// 删除考勤管理员
			delOrgAttendanceAdmin(userid);
			// 删除大厦考勤员
			delBuildingAtt(userid);
			...
			if(exp!=null){
				...
				new LDAPService().deleteUserByLoginUid(exp.getLoginUid());
			}
			res = true;
		}catch(Exception e){
			e.printStackTrace();
	...
	/**
	 * 删除T_013_LEADER_LEVEL 信息
	 * @param userid
	 */
	private void delLeaderLeavel(long userid){
		if( null != dao.findById(LeaderLevel.class, userid)){
			dao.deleteById(LeaderLevel.class, userid);
		}
	}
	
	/**
	 * 删除T_017_ATTENDANCE_APPROVERS 信息
	 * @param userid
	 */
	private void delAttendanceApprovers(long userid){
		if(null != dao.findById(AttendanceApprovers.class, userid)){
			dao.deleteById(AttendanceApprovers.class, userid);
		}
	}
	
	/**
	 * 删除T_019_ORG_ATTENDANCE_ADMIN 信息
	 * @param userid
	 */
	private void delOrgAttendanceAdmin(long userid){
		List<OrgAttendanceAdmin> orgAdminList = dao.findByProperty(OrgAttendanceAdmin.class, "userid", userid);
		if(!orgAdminList.isEmpty()){
			dao.delete(orgAdminList.get(0));
		}
	}
	
	/**
	 * 删除T_020_Building_Attendancers 信息
	 * @param userid
	 */
	private void delBuildingAtt(long userid){
		List<BuildingAttendancers> list = dao
				.findByHQL(
						"select aa from BuildingAttendancers aa where ',' || aa.userid || ',' like ?",
						"%,"+String.valueOf(userid)+",%");
		String[] atts = null;
		if(null != list && !list.isEmpty()){
			atts = list.get(0).getUserid().split(",");
			if(!ArrayUtils.isEmpty(atts)){
				// 删除该id
				atts = (String[]) ArrayUtils.removeElement(atts,String.valueOf(userid));
				// 重新组装id
				...
				if(sb.length() > 0){
					BuildingAttendancers buildingAttendancers = new BuildingAttendancers(2l,sb.substring(0, sb.length()-1));
					dao.deleteById(BuildingAttendancers.class, 2l);
					dao.save(buildingAttendancers);
				}
				else
				{
					dao.deleteById(BuildingAttendancers.class, 2l);
 ...
 */
	...
	* @throws
	...
	public String getOrgOtherUserCheckinData(String orgid, String type, String ctx) {
		if ("init".equals(type)) {
			Document doc = DocumentHelper.createDocument();
			Element root = doc.addElement("tree");
			root.addAttribute("id", "0");
			...
			Map pMap = new HashMap();
			pMap.put("userid", Struts2Util.getSession().getAttribute(Constants.USER_ID).toString());
			List<Org> list = this.dao.findByHQL("from Org o where o.orgId='1400' and o.orgId <> '2'");
			...
			for (Org org : list) {	            
				...
				Element elx = el.addElement("userdata");
				elx.addAttribute("name", "url");
				elx.addText(ctx+"/app/personnel/perInfoCheckinOther.act?orgId="+ org.getOrgId());		
	        }
...
//					TcIpTel  ttt = (TcIpTel)obj[2];
		            ...
		            Element elx = el.addElement("userdata");
		            elx.addAttribute("name", "url");            
		            elx.addText(ctx+"/app/per/inputRegisterOther.act?orgId="+orgId+"&userId="+ userId);	            
...
//			}
	...
	public String getOrgOtherUserCheckinData1(String orgid, String type, String ctx) {
		if ("init".equals(type)) {
			Document doc = DocumentHelper.createDocument();
			Element root = doc.addElement("tree");
			root.addAttribute("id", "0");
			...
			Map pMap = new HashMap();
			pMap.put("userid", Struts2Util.getSession().getAttribute(Constants.USER_ID).toString());
			List<Org> list = this.dao.findByHQL("from Org o where o.PId = '1' and o.orgId <> '2' and o.orgId = '1381'");
...
//			}
	...
	public String getOrgOtherUserCheckinDataMetro(String orgid, String type, String ctx) {
		if ("init".equals(type)) {
			Document doc = DocumentHelper.createDocument();
			Element root = doc.addElement("tree");
			root.addAttribute("id", "0");
			...
			Map pMap = new HashMap();
			pMap.put("userid", Struts2Util.getSession().getAttribute(Constants.USER_ID).toString());
			//List<Org> list = this.dao.findByHQL("from Org o where o.PId = '1' and o.orgId <> '2' and o.orgId = '1070'");
			List<Org> list = this.dao.findByHQL("from Org o where o.orgId = '1'");
...
//			}
	 ...
	 * 政务服务中心
	 */
	public String getOrgOtherUserCheckinDataAdminService(String orgid, String type, String ctx) {
		if ("init".equals(type)) {
			Document doc = DocumentHelper.createDocument();
			Element root = doc.addElement("tree");
			root.addAttribute("id", "0");
			...
			Map pMap = new HashMap();
			pMap.put("userid", Struts2Util.getSession().getAttribute(Constants.USER_ID).toString());
			List<Org> list = this.dao.findByHQL("from Org o where o.PId = '2' and o.orgId = '1069'");
