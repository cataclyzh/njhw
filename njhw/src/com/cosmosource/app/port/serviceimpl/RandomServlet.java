...
//		response.setDateHeader("Expires", 0);
		...
		try{
			//得到项目路径，主要是得到message.properties路径，加载此property
...
//			request.setAttribute("original", randNum);
		...
		json.put("original_data", randNum);
		json.put("original", randNum);
		json.put(KEY_APP_ID, props.get(KEY_APP_ID));
		json.put(KEY_AUTHURL, props.get(KEY_AUTHURL));
		response.getWriter().print(callbackparam+"("+json.toString()+")");
