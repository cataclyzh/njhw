...
import com.cosmosource.base.util.ServletUtil;
...
public class JCaptchaFilter implements Filter {
	...
	//context-param全局参数
	public static final String CONTEXT_PARAM_USER_JCAPTCHA = "userJCaptchaLogin";
	...
	protected void initParameters(final FilterConfig fConfig) {
		...
		useJCaptcha = fConfig.getServletContext().getInitParameter(CONTEXT_PARAM_USER_JCAPTCHA);
	...
	protected boolean validateCaptchaChallenge(final HttpServletRequest request) {
		try {
			...
			if("false".equals(useJCaptcha)){
				return true;
			}
			//自动通过值存在时,检验输入值是否等于自动通过值
			else if (StringUtils.isNotBlank(autoPassValue) && autoPassValue.equals(challengeResponse)) {
